import { Avatar, Box, Button, Center, HStack, Radio, ScrollView, Text } from "native-base";
		...
		<ScrollView bg={color.white}>
			<Center>
				<Box w="80%" mt="24px">
					...
					<Radio.Group defaultValue={friendId} onChange={(e) => setFriendId(e)}>
						{friendList.length !== 0 ?
								...
								return (
									<Radio key={index} value={item.userId} my={8}>
										<HStack w="100%" alignItems="center" space={4} ml="16px" py="16px">
											...
											/>
											<Text fontSize="18px">{item.displayName}</Text>
										</HStack>
									</Radio>
								)
							}) : null
