    ...
    <>
          ...
          return (
            ...
            >
              <HStack justifyContent="space-between">
                <Text fontWeight="medium" fontSize="16px">{data.topic}</Text>
                <Badge rounded="20px" variant="solid" w="70px" h="25px"
                  colorScheme={data.status === 'rejected' ? "error" : data.status === 'accepted' ? "success" : null}
                >
                  {data.status}
                </Badge>
              </HStack>
              <HStack alignItems="center">
                ...
                <Avatar size="8" marginX="8px"
                  source={{ uri: friendProfile || "http://placekitten.com/200/300" }} />
                <Text>{friendName}</Text>
              </HStack>
              <Text>Date: {data && getDateTime(data.dateTime, 'date')}</Text>
              <Text>Time: {data && getDateTime(data.dateTime, 'time')}</Text>
            </VStack>
          );
        }}
      </Pressable>
      ...
      <Modal isOpen={modalVisible} onClose={() => setModalVisible(false)}>
        <Modal.Content>
          ...
          <Modal.Body py="0" paddingBottom={data.status === "waiting" ? "0" : "16px"}>
            <VStack space={2}>
          ...
          {data.status === 'waiting' ?
            <Modal.Footer borderTopWidth="0" justifyContent="center">
              <Button.Group space={4}>
                <Button backgroundColor={color.green} rounded="20px" w="45%" onPress={() => onPressAccept()}>Accept</Button>
                <Button backgroundColor={color.red} rounded="20px" w="45%" onPress={() => onPressReject()}>Reject</Button>
              </Button.Group>
            </Modal.Footer> : null
          }
        </Modal.Content>
